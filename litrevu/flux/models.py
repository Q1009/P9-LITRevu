...
class Ticket(models.Model):
    ...
    image = models.ImageField(
        upload_to=ticket_image_path, verbose_name="Image", max_length=255)
    author = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, verbose_name="Auteur")
    date_created = models.DateTimeField(
        auto_now_add=True, verbose_name="Date de création")
    date_edited = models.DateTimeField(
        auto_now=False, null=True, blank=True, verbose_name="Date de modification")


class Review(models.Model):
    ticket = models.ForeignKey(
        to=Ticket, on_delete=models.CASCADE, verbose_name="Ticket")
